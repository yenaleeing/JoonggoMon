...
function uploadFormCheck(event) {
   ...
   // 이미지
   // 이미지 갯수 체크
    var imgLengthCount = document.querySelectorAll('.ui-state-default');
   if(imgLengthCount.length < 1) {
      alert("상품이미지가 없습니다.");
      return false;
   }
   // 제목
...
<body>
	...
	<div class="main-wrap">
		...
		<hr class="main-hr">
		<br>
		<form class="row g-3" action="/member/editProduct/edit.do" method="POST"   enctype="multipart/form-data">
			<!--상품이미지 파일업로드-->
					<!--상품이미지 파일업로드-->
			<div>
	...
	<div class="d-flex flex-row mb-3">
			...
			<div class="col-12">
				<button type="submit" class="btn bg-danger bg-gradient btn-submit"
					style="float: right; color: #fff" onclick="location.href='/member/editProduct/edit.do'">수정하기</button>
			</div>
			<hr>
		</form>
