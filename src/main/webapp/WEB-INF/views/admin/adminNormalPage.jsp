...
function deleteNotiBoard(){
	location.href='/admin/deleteBoard.do?board_id='+noti_board_id;
}
...
<%@ include file="/WEB-INF/views/admin/header/sidebar.jsp"%>
		...
		<%@ include file="/WEB-INF/views/admin/header/header.jsp"%>
				...
				<div class="container-fluid">
					...
					<!-- Table -->
					<div class="row">
						<form action="/admin/normalBoard" class="w-100" name="search"
								...
								<tbody>
										...
										<c:forEach var="notice" items="${list}">
											<tr>
												...
												<td>${notice.count}</td>
											<td>
												<button type="button" class="btn btn-outline-primary" onclick="location.href='/admin/normalBoard.do?board_id=${notice.board_id}' ">글 상세보기</button>
												<button type="button" class="btn btn-outline-warning"
													data-toggle="modal" data-target="#archiveModal">숨기기</button>
												<button type="button" class="btn btn-outline-danger"
													data-toggle="modal" data-target="#deleteModal" data-notifyid = "${notice.board_id}">삭제</button>
											</td>
											</tr>
										</c:forEach>
